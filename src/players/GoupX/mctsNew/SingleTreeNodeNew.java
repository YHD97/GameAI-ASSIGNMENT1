...
import players.GoupX.A_StarHeuristic;
import players.GoupX.CustomHeuristicNew;
...
public class SingleTreeNodeNew
{
    public MCTSParamsNew params;

    private SingleTreeNodeNew parent;
    private SingleTreeNodeNew[] children;
    ...
    SingleTreeNodeNew(MCTSParamsNew p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private SingleTreeNodeNew(MCTSParamsNew p, SingleTreeNodeNew parent, int childIdx, Random rnd, int num_actions,
        ...
        children = new SingleTreeNodeNew[num_actions];
        totValue = 0.0;
    ...
    void setRootGameState(GameState gs)
    {
        ...
        else if (params.heuristic_method == params.A_StarHeuristic) // New method: combined heuristics
            this.rootStateHeuristic = new A_StarHeuristic(gs, m_rnd);
    }
    ...
    void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        ...
        while(!stop){
            ...
            // 通过uct 选择出一个节点
            SingleTreeNodeNew selected = treePolicy(state);
    ...
    private SingleTreeNodeNew treePolicy(GameState state) {

        SingleTreeNodeNew cur = this;
    ...
    //expansion
    private SingleTreeNodeNew expand(GameState state) {
        ...
        SingleTreeNodeNew tn = new SingleTreeNodeNew(params,this,bestAction,this.m_rnd,num_actions,
    ...
    //uct算法 selection
    private SingleTreeNodeNew uct(GameState state) {
        SingleTreeNodeNew selected = null;
        double bestValue = -Double.MAX_VALUE;
        for (SingleTreeNodeNew child : this.children) // for 循环
        {
    ...
    //back propagation
    private void backUp(SingleTreeNodeNew node, double result)
    {
        SingleTreeNodeNew n = node;
        while(n != null)
        {
            n.nVisits++;
            n.totValue += result;
    ...
    private boolean notFullyExpanded() {
        for (SingleTreeNodeNew tn : children) {
            //找出子节点的值为null的
            if (tn == null) {
                return true;
            }
